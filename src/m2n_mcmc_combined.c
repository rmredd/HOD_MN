...
// TEST VALUES (ON ERRORS)
//double B_rozo = 1.09, B_rozo0 = 1.09, B_rozo_err = 0.01; //Mpivot = 2.06e13 Msol
//double alpha_rozo = 0.75, alpha_rozo0 = 0.75, alpha_rozo_err = 0.005;
//double sig_rozo = 0.35, sig_rozo0 = 0.35, sig_rozo_err = 0.02;
double PIVOT_MASS;
...
double bias_amp_err=0.06, //ORIGINAL VALUES
  mf_amp_err=0.05,
  scalebias_amp_err=0.15;
//double bias_amp_err=0.01, // TEST VALUES
//  mf_amp_err=0.01,
//  scalebias_amp_err=0.03;
  ...
  if(USE_ERRORS)
    {
      ...
      a[++i] = 1.0;
      start_dev[i] = scalebias_amp_err/5;
    }
  if(USE_VARIABLE_ROZO)
    {
      a[++i] = alpha_rozo0;
...
/*****************************
 * MCMC CHAIN POST-PROCESSING
 */

int mcmc_chain_post_processing(int ARGC, char **ARGV)
{
  double mean[200],disp[200], *xx;
  float dx,dy,x0,y0,grid[200][200],x1,x2,x3,x4,x5,x6,xhi,yhi,w,
    gmax=0,chi2[200][200],xmin=10000,xi,yi,xo,yo,x3min,x4min,xxmin[100];
  int nx,ny,n,i,j,i1,i2,np=5,ix,iy,j1,j2,nchains,npoints,ii,iweight,iprev,indx[130000],
    ncount=0,imax,jmax,nskip,imin,j1max=0,i1max=0, ntotal=0;
  FILE *fp,*fp1;
  char aa[1000],fname[10][100];

  int iprior[100];
  float prior_mu[100], prior_err[100];
  double wcount=0, s8original;
  double sum, mmin[4], fsat[4], mbar[4], merr[4], fbar[4], ferr[4];
  ...
  if(nchains<100)
    ...
    {
      nchains = 1;
      sprintf(fname[1],"%s",ARGV[3]);
    }

  nskip = npoints;
  for(ii=1;ii<=nchains;++ii)
    {
      fp1 = openfile(fname[ii]);
      n = filesize(fp1);
      npoints = n-nskip;
      for(j=1;j<=nskip;++j)
	fgets(aa,1000,fp1);
      ...
      for(i=1;i<npoints;++i)
	{
	  ...
	  xx[7] = pow(10.0,xx[7]);
	  xx[8] = pow(10.0,xx[8]);
	  xx[10] = pow(10.0,xx[10]);
	  xx[12] = pow(10.0,xx[12]);
	  xx[13] = pow(10.0,xx[13]);

	  // now put the parameters in place
	  for(j=1;j<=3;++j)
	    {
	      put_parameters_in_place(xx,j);
	      if(j==1) { SIGMA_8 = s8original*growthfactor(0.068); REDSHIFT=0.068; }
	      if(j==2) { SIGMA_8 = s8original*growthfactor(0.104); REDSHIFT=0.104; }
	      if(j==3) { SIGMA_8 = s8original*growthfactor(0.126); REDSHIFT=0.126; }
	      switch(j) {
	      case 1: GALAXY_DENSITY = 0.011134; break;
	      case 2: GALAXY_DENSITY = 0.0031634; break;
	      case 3: GALAXY_DENSITY = 0.0011486; break;
	      }
	      printf("HOD %e %e %e %e\n",HOD.M1,HOD.alpha,HOD.M_cut,HOD.sigma_logM);
	      printf("COSMO %e %e\n",OMEGA_M,SIGMA_8);
	      RESET_COSMOLOGY++;
	      ...
	      mmin[j] = HOD.M_min;
	      fsat[j] = qromo(func_satellite_density, log(HOD.M_low),log(HOD.M_max),midpnt)/GALAXY_DENSITY;
	      mbar[j] += log10(HOD.M_min);
	      fbar[j] += fsat[j];
	      merr[j] += log10(HOD.M_min)*log10(HOD.M_min);
	      ferr[j] += fsat[j]*fsat[j];
	    }
	  printf("POST %d %d %e %e %e %e %e %e\n",i1,i2,mmin[1],fsat[1],mmin[2],fsat[2],mmin[3],fsat[3]);
	  fflush(stdout);
	}
    }
  ...
  for(j=1;j<=3;++j)
    {
      fbar[j] = fbar[j]/ntotal;
      ferr[j] = sqrt(ferr[j]/ntotal - fbar[j]*fbar[j]);
      mbar[j] = mbar[j]/ntotal;
      merr[j] = sqrt(merr[j]/ntotal - mbar[j]*mbar[j]);
      fprintf(stderr,"%e %e %e %e\n",fbar[j],ferr[j],mbar[j],merr[j]);
    }
